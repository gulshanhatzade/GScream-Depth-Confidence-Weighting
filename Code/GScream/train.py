...
def generate_depth_confidence_map(depth_map, edge_threshold=0.5):
    """
    Generate confidence map based on depth discontinuities.
    Lower confidence near edges, higher confidence in smooth regions.
    
    Args:
        depth_map: Tensor of shape (B, H, W)
        edge_threshold: Threshold for edge detection sensitivity
        
    Returns:
        confidence_map: Tensor of shape (B, H, W) with values between 0 and 1
    """
    ...
    for b in range(batch_size):
        ...
        depth_img = (depth_np[b] - depth_np[b].min()) / (depth_np[b].max() - depth_np[b].min() + 1e-8)
        ...
        # Convert to confidence (higher at smooth regions, lower at edges)
        confidence = 1.0 - (gradient_magnitude > edge_threshold).astype(np.float32)
    ...
    confidence_tensor = torch.from_numpy(np.stack(confidence_maps)).to(depth_map.device)
    return confidence_tensor

def save_confidence_visualization(confidence_map, iteration, save_path):
    """
    Visualize and save confidence maps
    """
    os.makedirs(save_path, exist_ok=True)
    
    # Create a heatmap visualization
    heatmap = plt.cm.jet(confidence_map.detach().cpu().numpy()[0])[:,:,:3]
    heatmap = torch.from_numpy(heatmap).permute(2,0,1)
    
    torchvision.utils.save_image(
        heatmap, 
        os.path.join(save_path, f"confidence_{iteration:05d}.png")
    )
    
    # Also save the raw confidence values for analysis
    np.save(os.path.join(save_path, f"confidence_{iteration:05d}.npy"), 
            confidence_map.detach().cpu().numpy())
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if opt.refer_depth_lr > 0 and gt_image_name == ref_name:
            ...
            confidence_vis_path = os.path.join(dataset.model_path, "confidence_maps")
            os.makedirs(confidence_vis_path, exist_ok=True)
            
            # Visualize confidence map periodically
            if iteration % 100 == 0:
                save_confidence_visualization(confidence_map, iteration, confidence_vis_path)
            
            # Apply confidence to valid mask
            weighted_mask = valid_mask * confidence_map
            
            scale, shift = compute_scale_and_shift(depth, midas_depth, weighted_mask)
            ...
            loss += opt.refer_depth_lr * l1_loss_masked(aligned_depth, midas_depth, weighted_mask)
            ...
            if opt.refer_depth_lr_fg > opt.refer_depth_lr:
                ...
                # Apply confidence weighting to fg_mask as well
                fg_weighted_mask = fg_mask * confidence_map
                loss += (opt.refer_depth_lr_fg - opt.refer_depth_lr) * l1_loss_masked(aligned_depth, midas_depth, fg_weighted_mask)
            
            for scale in range(4):
                step = pow(2, scale)
                confidence_scaled = confidence_map[:,::step,::step] if step > 1 else confidence_map
                weighted_mask_scaled = torch.ones_like(gt_mask)[:,::step,::step] * confidence_scaled
                loss += 0.5 * opt.refer_depth_lr_smooth * gradient_loss(aligned_depth[:,::step,::step], midas_depth[:,::step,::step], weighted_mask_scaled)
        ...
        if opt.other_depth_lr > 0 and gt_image_name != ref_name:
            ...
            # Generate depth confidence map for other views too
            confidence_map = generate_depth_confidence_map(depth)
            ...
            # Use confidence-weighted loss
            loss += opt.other_depth_lr * l1_loss_masked(aligned_depth, midas_depth, weighted_mask)

            for scale in range(4):
                step = pow(2, scale)
                confidence_scaled = confidence_map[:,::step,::step] if step > 1 else confidence_map
                weighted_mask_scaled = valid_mask[:,::step,::step] * confidence_scaled
                loss += 0.5 * opt.other_depth_lr_smooth * gradient_loss(aligned_depth[:,::step,::step], midas_depth[:,::step,::step], weighted_mask_scaled)
